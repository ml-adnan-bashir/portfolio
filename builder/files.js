...
async function open(folder){const files={};for(const name of ['index.html','projects.html','portfolio-design.json','projects-data.json','builder-defaults.js','.gitignore']){const f=await read(folder,name);files[name]=f?await f.text():null;}if(!files['index.html']||!files['projects.html'])throw Error('Select the portfolio folder containing both index.html and projects.html.');return {folder,expected:files,sources:{home:files['index.html'],projects:files['projects.html']}};}
